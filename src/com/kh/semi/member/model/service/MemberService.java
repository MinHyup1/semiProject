...
public class MemberService {
	 ...
	 * 
	 * MailSender mailSender = new MailSender(); HttpConnector conn = new
	 * HttpConnector(); //Http 통신 관련된 역할 수행해줄 클래스 만들어서 불러오기
	 * 
	 * String queryString = conn.urlEncodedForm(RequestParams .builder()
	 * .param("mailTemplate", "join-auth-mail") .param("userId", member.getUserId())
	 * .param("persistToken", persistToken).build());
	 * 
	 * String response = conn.get("http://localhost:9090/mail?" + queryString);
	 * mailSender.sendEmail(member.getEmail(), "회원가입을 축하합니다.", response); //사용자가 입력한
	 * 이메일로 보내도록
	 * 
	 * }
	 */
